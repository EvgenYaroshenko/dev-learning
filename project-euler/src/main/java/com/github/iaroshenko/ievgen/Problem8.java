...
public class Problem8 {
    ...
    public static BigInteger consecutiveProduct(String number, int i) {
        int cursor = 0;
        ...
        if (i > number.length()) {
            throw new NumberFormatException("i must be lesser or equal number length");
        }

        while (cursor <= number.length() - i) {
            System.out.println(cursor);
            boolean cursorBreaked = false;
            for (int j = cursor; j < cursor + i; j++) {
                if (number.charAt(j) == '0') {
                    int curEl = j;
                    while (curEl <= number.length() - i && number.charAt(curEl) == '0') {
            ...
            if (!cursorBreaked) {
                ...
                if (previousResult.equals(BigInteger.ZERO)) {
                    previousResult = BigInteger.ONE;
                    for ( int j =  cursor; j < cursor + i; j++) {
                        previousResult = previousResult.multiply(BigInteger.valueOf(number.charAt(j) - '0'));
                    }
...
//                        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(consecutiveProduct(text, 13));
    }
}
